...
class TwoTowerModel(tfrs.models.Model):
  ...
  def __init__(self, user_tower, item_tower, product_features):
    super().__init__()
    ...
    # User tower
    self.user_model = models.Sequential()
    self.user_model.add(self.user_tower)
    self.user_model.add(layers.Dense(64, activation="relu"))
    self.user_model.add(layers.Dense(32, activation="relu"))
    self.user_model.add(layers.Dense(32))

    # Item tower
    self.item_model = models.Sequential()
    self.item_model.add(self.item_tower)
    self.item_model.add(layers.Dense(64, activation="relu"))
    self.item_model.add(layers.Dense(32, activation="relu"))
    self.item_model.add(layers.Dense(32))

    # Retrieval task for loss function
    metrics = tfrs.metrics.FactorizedTopK(
            candidates=product_features.batch(128).map(self.item_tower)
    )
